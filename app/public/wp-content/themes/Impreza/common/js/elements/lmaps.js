...
!function( $ ) {
	...
	$us.WLmaps.prototype = {
		...
		redraw: function() {
			var self = this;
			if ( ! self.lmap || self.$container.is( ':hidden' ) ) {
				return;
			}
			$us.timeout( self.lmap.invalidateSize.bind( self.lmap, /* animate */true ), 100 );
		},
		...
		renderMap: function() {
			...
			if ( $us.usbPreview() || this.options.disableDragging && ( ! $us.$html.hasClass( 'no-touch' ) ) ) {
