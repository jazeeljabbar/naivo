 ...
 *  Review request
 * 
 *  @since  2.0.8   Showing review banner based on created coupon count
 *
...
class Wt_Smart_Coupon_Review_Request
{
    ...
    private $review_url                 =   "https://www.webtoffee.com/testimonials/?product_id=85057#submit-your-review";
    private $plugin_prefix              =   "wt_smart_coupon_pro"; /* must be unique name */
    ...
    public function init()
    {
        ...
        {
            $this->banner_message = sprintf( __( "Hey, %s We at %sWebToffee%s would like to thank you for using %s Smart Coupon for WooCommerce %s plugin! %s We would really appreciate if you could take a moment to leave a quick review—it will inspire us to keep improving.", 'wt-smart-coupons-for-woocommerce-pro' ), '<br />', '<b>', '</b>', '<b>', '</b>', '<br />' );

            if( $this->created_count > $this->required_created_count )
            {
                $this->banner_message = sprintf( __( '%s Wow, You’ve created over %s coupons with our %s %s Smart Coupon for WooCommerce %s %s plugin! %s 🎉 %s That’s amazing! If you’re enjoying it, we’d love a quick rating—your feedback helps us improve and reach more users.', 'wt-smart-coupons-for-woocommerce-pro' ), '<span>', '<b>'.absint( $this->required_created_count ).'</b>', '</span>', '<b>', '</b>', '<span>', '</span>', '<br />' );
            }

            /* button texts */
            $this->later_btn_text   = __( "Remind me later", 'wt-smart-coupons-for-woocommerce-pro' );
            $this->never_btn_text   = __( "Not interested", 'wt-smart-coupons-for-woocommerce-pro' );
            $this->review_btn_text  = __( "Rate us now", 'wt-smart-coupons-for-woocommerce-pro' );
    ...
    public function set_vars()
    {
        ...
        $this->webtoffee_logo_url           =    WT_SMARTCOUPON_URL . 'admin/images/review_banner_bg.webp';
    ...
    public function add_banner_scripts()
    {
        ...
        ?>
        <style type="text/css">
            .wt_smart_coupon_pro_review_request{ border-left-color:#FFE500; background:linear-gradient(to right, #fcf5bc, #fff) #e8e0a6; padding-right:0px; padding-bottom:0px; }
            .wt_smart_coupon_pro_review_request b{ color:#FF6636; font-size:14px; }
            .wt_smart_coupon_pro_review_request span{ font-weight:bold; font-size:14px; }
            .wt_smart_coupon_pro_review_request .button-primary{ background:#FFE500; border-color:#ccc; color:#000; }
        </style>
