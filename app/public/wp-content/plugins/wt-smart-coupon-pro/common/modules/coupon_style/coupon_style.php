...
class Wt_Smart_Coupon_Style 
{
    ...
    public function __construct()
    {
        ...
        add_filter('wt_sc_alter_coupon_email_css', array($this, 'add_css_to_email'), 10, 1);
    }
    ...
    */
    ...
    protected static function migrate_settings()
    {
        ...
        if(isset($smart_coupon_option['wt_coupon_styles']) && !empty($smart_coupon_option['wt_coupon_styles'])) /* old data exists */
        {
            /* fix style name typo */
            if(is_array($smart_coupon_option['wt_coupon_styles']))
            {
                foreach($smart_coupon_option['wt_coupon_styles'] as $coupon_type=>$coupon_style)
                {
                    if(isset($coupon_style['style']) && 'plane_coupon' === $coupon_style['style'])
                    {
                        $smart_coupon_option['wt_coupon_styles'][$coupon_type]['style']='plain_coupon';
                    }
                }
            }

            Wt_Smart_Coupon::update_settings($smart_coupon_option['wt_coupon_styles'], self::$module_id_static);

            //remove old option
            unset($smart_coupon_option['wt_coupon_styles']);
    ...
    {
        $coupon_types = array(
            'available_coupon'  =>  __('Available coupon', 'wt-smart-coupons-for-woocommerce-pro'),
            'used_coupon'       =>  __('Used coupon', 'wt-smart-coupons-for-woocommerce-pro'),
            'expired_coupon'    =>  __('Expired coupon', 'wt-smart-coupons-for-woocommerce-pro'),
        );
     ...
     * Print coupon HTML
     *
     * @deprecated 2.0.7 In favor of Wt_Smart_Coupon_Style::prepare_coupon_html.
     * 
     */
     ...
     *  
     *  @since  2.0.7
     *  @since  2.0.8       Removed `click to apply` title and pointer cursor from coupon block in email.
        ...
        if("" !== $html)
        {
            ...
            switch($coupon_type)
            {
                ...
                default :
                    ...
                    $find_replace['[wt_sc_single_coupon_title]'] = esc_attr__('Click to apply coupon', 'wt-smart-coupons-for-woocommerce-pro');                   
            }
            ...
            if($is_preview_mode) //in preview mode, add dummy values
            {
                ...
                $find_replace['[wt_sc_coupon_type]']            = wp_kses_post(isset($coupon_data['coupon_type']) ? $coupon_data['coupon_type'] : __('Cart discount', 'wt-smart-coupons-for-woocommerce-pro'));
            ...
            }else
            {
                ...
                if( 'email_coupon' !== $coupon_type && class_exists( 'Wbte_Smart_Coupon_Bogo_Common' ) && method_exists( 'Wbte_Smart_Coupon_Bogo_Common', 'is_bogo' ) && Wbte_Smart_Coupon_Bogo_Common::is_bogo( $coupon_id ) ){
                    ...
                    if( apply_filters( 'wbte_sc_bogo_type_instead_description', false, $coupon_id ) || empty( $bogo_description ) ){
                        ...
                        if( 'wbte_sc_bogo_bxgx' === get_post_meta( $coupon->get_id(), 'wbte_sc_bogo_type', true ) ) {
                            $find_replace['[wt_sc_coupon_code]'] = esc_html__( 'Buy X Get X/Y', 'wt-smart-coupons-for-woocommerce-pro' );
                        }else{
                            ...
                            $find_replace['[wt_sc_coupon_code]'] = esc_html( $_cheap_exp_txt[ get_post_meta( $coupon->get_id(), 'wbte_sc_bogo_customer_gets_cheap_exp', true ) ] );
                        }
                    }
                    else{
                ...
                if(isset($coupon_data['display_on_page']) && 'credit_meta' !== $coupon_data['display_on_page'] && $coupon->is_type('store_credit'))
                {
                    $find_replace['[wt_sc_credit_history_url]'] = esc_attr(Wt_Smart_Coupon_Public::get_store_credit_url($coupon_id));   
                    $find_replace['[wt_sc_credit_history]'] = '&nbsp;';   //to prevent hiding 
                }else
                {
                    $find_replace['[wt_sc_credit_history_url]'] = '';
            ...
            /** @since 2.3.0    Add coupon code for attribute. */
    ...
    private static function hide_empty_elements($find_replace, $html, $coupon_type)
    {
        $elements_to_hide = array(
            'wt_sc_coupon_start', 'wt_sc_coupon_expiry', 'wt_sc_credit_history', 'wt_sc_coupon_desc_wrapper', 'wt_sc_coupon_amount',
        ); 
    ...
    {
        return (is_array($match) && isset($match[1]) && "" !== trim($match[1]) ? __($match[1], 'wt-smart-coupons-for-woocommerce-pro') : '');
    }
    ...
    /**
     *  Callback function to add CSS styles for coupon email
     * 
     *  @since    2.0.7  
     *  @param  string    $css  Email CSS 
     *  @return  string    $css  CSS including coupon styles
     */
    public function add_css_to_email($css)
    {
        return $css.self::get_coupon_default_css();
    }
}
