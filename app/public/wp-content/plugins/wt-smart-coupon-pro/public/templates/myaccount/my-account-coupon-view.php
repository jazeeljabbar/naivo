 ...
 */
do_action_deprecated('wt_smart_coupon_before_my_coupons', array(), '2.0.0', 'wt_smart_coupon_before_my_account_coupons');

do_action('wt_smart_coupon_before_my_account_coupons', $current_user);
?>
<div class="wt-mycoupons">
    <h4><?php _e('Available Coupons', 'wt-smart-coupons-for-woocommerce-pro'); ?></h4>
    <div class="wt_sc_available_coupon_sort_by">
        <form>
            <span><?php _e('Sort by', 'wt-smart-coupons-for-woocommerce-pro'); ?> </span>
            ...
            <select name="wt_sc_available_coupons_orderby" onchange="this.form.submit()">
                <option value="created_date:desc" <?php selected("created_date:desc", $orderby); ?>><?php _e('Latest first', 'wt-smart-coupons-for-woocommerce-pro'); ?></option>
                <option value="created_date:asc" <?php selected("created_date:asc", $orderby); ?>><?php _e('Latest last', 'wt-smart-coupons-for-woocommerce-pro'); ?></option>
                <option value="amount:desc" <?php selected("amount:desc", $orderby); ?>><?php _e('Price high to low', 'wt-smart-coupons-for-woocommerce-pro'); ?></option>
                <option value="amount:asc" <?php selected("amount:asc", $orderby); ?>><?php _e('Price low to high', 'wt-smart-coupons-for-woocommerce-pro'); ?></option>
            </select>
        </form>
    </div>
    <?php
...
$show_used_coupons = in_array( 'used_coupons', Wt_Smart_Coupon::get_option( 'wbte_sc_coupons_page_additional_display' ) ); 
if($show_used_coupons)
{
?>
    <div class="wt-used-coupons">
        ...
        if(!empty($used_coupons))
        {
            ...
            foreach($used_coupons as $coupon)
            {
                ...
                if(!is_a($coupon_post, 'WP_Post') || (isset($coupon_post->post_status) && 'publish' !== $coupon_post->post_status)) //post status is not published
                {
                    continue;
                }
                ...
                if(0 === $i)
                {
                    ...
                    echo Wt_Smart_Coupon_Public::get_coupon_html($coupon_post, $coupon_data, 'used_coupon', true);
            ...
            if(0 < $i) /* close the coupon wrapper div */
            {
        ...
        }else
        {
            echo '<div class="wt_sc_myaccount_no_used_coupons">';
                echo apply_filters('wt_sc_alter_myaccount_no_used_coupons_msg', __("Sorry, you don't have any used coupons", 'wt-smart-coupons-for-woocommerce-pro'));
            echo '</div>';
        }
...
$show_expired_coupon = in_array( 'expired_coupons', Wt_Smart_Coupon::get_option( 'wbte_sc_coupons_page_additional_display' ) );

if($show_expired_coupon)
{
?>
    <div class="wt-expired-coupons">
        ...
        if(!empty($post_ids))
        {
            ...
            foreach($post_ids as $post_id)
            {
                ...
                if(0 === $e)
                {
                    echo Wt_Smart_Coupon_Public::get_coupon_html($post, $coupon_data, 'expired_coupon', true);
                }else
                {
        ...
        }else
        {
            echo '<div class="wt_sc_myaccount_no_expired_coupons">';
                echo apply_filters('wt_sc_alter_myaccount_no_expired_coupons_msg', __("Sorry, you don't have any expired coupons", 'wt-smart-coupons-for-woocommerce-pro'));
            echo '</div>';
        }
