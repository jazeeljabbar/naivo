	...
	foreach ( $free_products as $coupon_code => $free_product_items ) {
		...
		<ul class="woocommcerce wbte_sc_bogo_products" coupon="<?php echo esc_attr( $coupon_id ); ?>">
			...
			foreach ( $free_product_items as $product_id ) {
				...
				$_data_item_key = isset( $triggered_item_arr[ $coupon_code ][ $product_id ] ) ? 'data-triggered-item-key="' . esc_attr( $triggered_item_arr[ $coupon_code ][ $product_id ] ) . '"' : '';
				?>
				<li class="wbte_get_away_product" title="<?php echo esc_attr( $_product->get_name() ); ?>" data-is_purchasable="<?php echo esc_attr( $is_purchasable ? 1 : 0 ); ?>" product-id="<?php echo esc_attr( $temp_product_id ); ?>" data-free-qty="<?php echo esc_attr( $free_products_qty[ $coupon_code ][ $product_id ] ); ?>" <?php echo esc_attr( $_data_item_key ); ?>>
					...
					<div class="wbte_product_discount">
						...
						<?php
						echo esc_html__( 'Quantity : ', 'wt-smart-coupons-for-woocommerce-pro' );
						?>
					...
					?>
					<button class="wbte_choose_free_product" prod-id="<?php echo esc_attr( $product_id ); ?>" type="button"><?php esc_html_e( 'Choose product', 'wt-smart-coupons-for-woocommerce-pro' ); ?></button>
				</li>
				<?php
			}
			?>
