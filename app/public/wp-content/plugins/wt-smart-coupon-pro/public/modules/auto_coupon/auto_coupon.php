...
class Wt_Smart_Coupon_Auto_Coupon_Public extends Wt_Smart_Coupon_Auto_Coupon_Common
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_loaded', array($this, 'maybe_auto_apply_on_wp_loaded'),11);
     ...
     * @since 1.1.0
     * @since 2.0.2 Updated the code to validate the coupon in single SQL 
     * @since 2.0.6 Saving the already fetched coupons list to avoid multiple DB call 
    ...
    public function set_session($key, $value)
    {
        if (!isset($this->_session_data)) {
            ...
            $this->_session_data = !is_null( WC()->session ) ? WC()->session->get( '_wt_smart_coupon_session_data', array() ) : array() ;
        } 
     ...
     *  @since 1.1.0
     *  @since 2.0.5  [Bug fix] Removes valid autocoupons 
    ...
    {        
        ...
        //Value assigned from the giveaway public module. Value will be 1 when the product is currently in the process of adding to the cart and the coupon has 'apply cheapest' enabled.
        if( !empty( $wbte_sc_cheapest_currently_adding ) && 1 === $wbte_sc_cheapest_currently_adding ){
            return;
        }
     ...
     *  Auto apply coupons
     *  @since 1.1.0
     *  @since 2.0.5  [Bug fix] Conflict with `auto apply` and `coupon individual use`. 
     *  @since 2.0.6  [Bug fix] Not automatically applying coupons with wild card(*) email restriction. 
            ...
            foreach($auto_coupons as $coupon_code => $coupon)
            {
                ...
                {                                         
                    ...
                    */
                    ...
                    if(Wt_Smart_Coupon_Public::module_exists('notifications'))
                    {
                       $message = Wt_Smart_Coupon_Notifications_Public::get_coupon_applied_message($coupon_code, $message);
                    }

                    $new_message = apply_filters('wt_smart_coupon_auto_coupon_message', $message, $coupon);                               
    ...
    public function coupon_html($originaltext, $coupon)
    {
        if ($this->is_auto_coupon($coupon)) {
            ...
            $auto_applied_coupons = !is_null( WC()->session ) ? WC()->session->get( 'auto_applied_coupons', array() ): array() ;
            if( ! in_array( $coupon->get_code(), $auto_applied_coupons, true ) ){
                return $originaltext;
            }
            ...
            if ($coupon->get_free_shipping()) {
                $value[] = __('Free shipping coupon', 'wt-smart-coupons-for-woocommerce-pro');
            }
    ...
    */
    ...
    private function get_applied_auto_coupon_ist() {      
        return ( isset( WC()->session ) ? WC()->session->get( 'auto_applied_coupons', array() ) : array() );
    }
    ...
    public function add_individual_coupons_to_session( $coupon_code ){
        ...
        if( $coupon->get_individual_use() ){
            ...
            if( class_exists( 'Wbte_Smart_Coupon_Bogo_Public' ) && method_exists( 'Wbte_Smart_Coupon_Bogo_Public', 'check_any_free_products_without_coupon' ) ){
                Wbte_Smart_Coupon_Bogo_Public::get_instance()->check_any_free_products_without_coupon();
            }
        }
    }
     ...
     *  An applied individual coupon exists in the session. @since 2.4.2
     *  'wbte_sc_auto_applied_coupons_fetched' is true, already fetched list making error in removing auto coupons. @since 2.4.3
     *  
     *  @since  2.4.2
     *  @since  2.4.3         Return false when 'wbte_sc_auto_applied_coupons_fetched' is true.
    ...
    public function remove_individual_coupons_from_session( $arg ){
        if ( !is_null( WC()->session ) && WC()->session->get( 'wbte_sc_applied_individual_coupon', false ) === $arg) {
