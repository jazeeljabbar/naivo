 ...
 */
class Wt_Sc_Blocks_Integration implements IntegrationInterface {
		...
		foreach ( $this->registered_blocks as $block_data ) {
			...
			if ( is_array( $block_data ) 
				...
				if ( in_array( 'editor-js' , $block_data['script_handles'] ) ) {
					$script_url        = WT_SC_BLOCKS_URL . 'build/' . $block_data['block_dir'] . '/index.js';
					$script_asset_path = WT_SC_BLOCKS_MAIN_PATH . 'build/' . $block_data['block_dir'] . '/index.asset.php';
				...
				if ( in_array( 'editor-css' , $block_data['script_handles'] ) ) {
					...
					$style_url = WT_SC_BLOCKS_URL . 'build/' . $block_data['block_dir'] . '/index.css';
					$style_path = WT_SC_BLOCKS_MAIN_PATH . 'build/' . $block_data['block_dir'] . '/index.css';
	...
	public function register_block_frontend_scripts() {
		...
		foreach ( $this->registered_blocks as $block_data ) {
			...
			if ( is_array( $block_data ) 
				...
				$script_url        = WT_SC_BLOCKS_URL . 'build/' . $block_data['block_dir'] . '/frontend.js';
				$script_asset_path = WT_SC_BLOCKS_MAIN_PATH . 'build/' . $block_data['block_dir'] . '/frontend.asset.php';
				...
				$handle = 'wt-sc-blocks-' . $block_data['block_dir'] . '-frontend';
				wp_enqueue_script(
					$handle,
					$script_url,
					$script_asset['dependencies'],
				...
				$block_lang_path = WP_CONTENT_DIR . '/languages/plugins/wt-smart-coupon-pro/blocks/';
				//To translate strings in blocks.
				wp_set_script_translations( $handle, 'wt-smart-coupons-for-woocommerce-pro', $block_lang_path );
			}
		}
	}	
}