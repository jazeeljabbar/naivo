...
if ( ! defined( 'WT_SC_BLOCKS_FILE' ) ) {
    define( 'WT_SC_BLOCKS_FILE', __FILE__ );
}

if ( ! defined( 'WT_SC_BLOCKS_MAIN_PATH' ) ) {
    define( 'WT_SC_BLOCKS_MAIN_PATH', plugin_dir_path( __FILE__ ) );
}

if ( ! defined( 'WT_SC_BLOCKS_URL' ) ) {
    define( 'WT_SC_BLOCKS_URL', plugin_dir_url( __FILE__ ) );
}

if ( ! defined( 'WT_SC_BLOCKS_VERSION' ) ) {
    define( 'WT_SC_BLOCKS_VERSION', '1.0.0' );
}
...
if ( ! class_exists( 'Wt_Sc_Blocks' ) ) {
	...
	class Wt_Sc_Blocks {
		...
		public function init() {			
			...
			include_once WT_SC_BLOCKS_MAIN_PATH . 'includes/class-wt-sc-blocks-integration.php';       
			...
			$registered_blocks = array(
				'main' => array(
	            	...
	            	'post_fields_schema' => array(
	            		'coupon_code'  => array(
			 						'description' => __( 'Coupon code', 'wt-smart-coupons-for-woocommerce-pro' ),
			 						'type'        => array( 'string', 'null' ),
			 						'readonly'    => true,
			 					),
	            		'coupon_message'  => array(
			 						'description' => __( 'Coupon message', 'wt-smart-coupons-for-woocommerce-pro' ),
			 						'type'        => array( 'string', 'null' ),
			 						'readonly'    => true,
			 					),
	            	),
				...
				if ( is_array( $block_data ) && isset( $block_data['block_dir'] ) ) {
					...
					if ( isset( $block_data['php_include'] ) && is_array( $block_data['php_include'] ) ) {				
						foreach ( $block_data['php_include'] as $file ) {							
							$file_path = WT_SC_BLOCKS_MAIN_PATH . 'src/' . $block_data['block_dir'] . '/' . $file;
		...
		public function register_checkout_blocks( $integration_registry ) {
		    ...
		    if ( ! empty( $this->registered_blocks ) && class_exists( 'Wt_Sc_Blocks_Integration' ) ) { // Blocks available.
		        $wt_sc_blocks_integration = new Wt_Sc_Blocks_Integration();
	        	...
	        	woocommerce_store_api_register_update_callback(
				    array(
				      ...
				      'callback'  => function( $data ) {
				      		...
				      		// Check and return coupon applied message. Return the message if custom message is available.
							if ( 
								isset( $data['coupon_action'] ) && 'apply' === $data['coupon_action'] 
								&& isset( $data['coupon_code'] ) && "" !== wc_sanitize_coupon_code($data['coupon_code'])
								&& class_exists( 'Wt_Smart_Coupon_Notifications_Public' )  && method_exists( 'Wt_Smart_Coupon_Notifications_Public', 'get_coupon_applied_message' ) 
							) {
								$this->block_post_fields['coupon_message'] = Wt_Smart_Coupon_Notifications_Public::get_coupon_applied_message( wc_sanitize_coupon_code($data['coupon_code']) );
							}
				      },
				    )
			  	);
