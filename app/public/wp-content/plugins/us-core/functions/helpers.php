...
if ( ! function_exists( 'us_get_public_post_types' ) ) {
	 ...
	 */
	function us_get_public_post_types( $exclude = NULL ) {
	...
	function us_get_page_area_id( $area, $page_args = array() ) {
		...
		if (
			...
			if ( $current_post_ID AND metadata_exists( 'post', $current_post_ID, 'us_' . $area . '_id' ) ) {
				...
				// then check if the value has ID of non-existing Reusable Block (if it was deleted)
...
if ( ! function_exists( 'us_maybe_get_button_style' ) ) {
	/**
	 * Check Button Styles created on Theme Options > Button Styles
	 *
	 * @param int $style
	 * @return int
	 */
	function us_maybe_get_button_style( $style = 1 ) {
		$btn_styles = us_get_btn_styles();

		if ( ! empty( $btn_styles ) AND ! array_key_exists( $style, $btn_styles ) ) {
			$style = array_key_first( $btn_styles );
		}
...
if ( ! function_exists( 'us_generate_link_atts' ) ) {
	...
	function us_generate_link_atts( $link = '' ) {
		...
		// Check for custom fields values like `{{field_name}}` or `{{option/field_name}}`
		if ( isset( $result['url'] ) AND preg_match( "#{{([^}]+)}}#", trim( rawurldecode( $result['url'] ) ), $matches ) ) {
			global $us_grid_term;

			// https://www.advancedcustomfields.com/resources/get-values-from-an-options-page/
			if (
				preg_match( '/^option(\/|\|)/', $matches[1] )
				AND function_exists( 'us_acf_get_field_object' )
			) {
				$acf_field = (array) us_acf_get_field_object( $matches[1], 'option' );

				if ( isset( $acf_field['value'] ) AND is_array( $acf_field['value'] ) ) {
					$result['url'] = us_arr_path( $acf_field, 'value.url', '' );
				} else {
					$result['url'] = us_arr_path( $acf_field, 'value', '' );
				}
				...
				if ( $meta_value = get_post_meta( get_the_ID(), $matches[1], TRUE ) ) {
					...
					if ( is_array( $meta_value ) ) {
						...
						try {
							...
							if ( is_array( $meta_value_array ) ) {
								$result['url'] = $meta_value_array['url'];

								// Override "target" only if it was empty
								if ( empty( $result['target'] ) AND ! empty( $meta_value_array['target'] ) ) {
									$result['target'] = $meta_value_array['target'];
								}
		...
		if ( ! empty( $result['url'] ) ) {
			...
			if ( strpos( rawurldecode( $result['url'] ), '[lang]' ) !== FALSE ) {
				...
				$result['url'] = str_replace( '[lang]', $replacer, rawurldecode( $result['url'] ) );
			}

			// If the URL is email, add "mailto:"
			if ( is_email( $result['url'] ) ) {
				$result['href'] = 'mailto:' . $result['url'];
		...
		return apply_filters( 'us_generate_link_atts', array_map( 'trim', $result ) );
	}
}
	...
	function us_get_elm_link_options() {
		...
		$link_options = array(
			__( 'Custom appearance in Grid', 'us' ) => array(
				'us_tile_link' => __( 'Custom Link', 'us' ),
			),
		);

		// Add Testimonial author link, if Testimonials are enabled
		if ( us_get_option( 'enable_testimonials', 1 ) ) {
			$link_options[ __( 'Testimonial', 'us' ) ][ 'us_testimonial_link' ] = __( 'Author Link', 'us' );
		}
		...
		if (
			...
			$allowed_types = array( 'url', 'link', 'page_link', 'file', 'email' );
			$link_options += (array) us_acf_get_fields( $allowed_types, /* to_list */TRUE, /* separator */'/' );
		}
...
if ( ! function_exists( 'us_output_design_css' ) ) {
	...
	function us_output_design_css( $custom_posts = [] ) {
		...
		if ( is_home() AND $posts_page_ID = us_get_option( 'posts_page' ) ) {
...
if ( ! function_exists( 'us_get_responsive_states_to_js' ) ) {
	/**
	 * Get responsive states and formation output to JS
	 *
	 * @param false $json
	 * @return array|string
	 */
	function us_get_responsive_states_to_js( $json = FALSE ) {
		$laptops_int = (int) us_get_option( 'laptops_breakpoint' );
		$tablets_int = (int) us_get_option( 'tablets_breakpoint' );
		$mobiles_int = (int) us_get_option( 'mobiles_breakpoint' );
...
if ( ! function_exists( 'us_get_responsive_values' ) ) {
	...
	function us_get_responsive_values( $value ) {
		$result = array();
		if (
			$value = json_decode( rawurldecode( $value ), /* return as array */TRUE )
			AND is_array( $value )
		) {
			$result = array();
			...
			foreach ( $map_values as $mask_name => &$mask_props ) {
				if ( count( $mask_props ) === count( $css_mask[ $mask_name ] ) OR $mask_name == 'background' ) {
					...
					if ( strpos( $mask_name, '-shadow' ) !== FALSE ) {
						...
						if ( array_sum( array_map( 'abs', array_map( 'floatval', $_value ) ) ) === 0.0 ) {
							continue;
						}
					}
...
if ( ! function_exists( 'us_replace_dynamic_value' ) ) {
	 ...
	 *
	 * @param string $string
	 * @param string $us_elm_context : shortcode / grid / header
	 * @return string
	 */
	function us_replace_dynamic_value( $string, $us_elm_context = NULL ) {
		if ( ! is_string( $string ) ) {
			return '';
		}
		...
		if ( ! empty( $string ) AND preg_match( $pattern, $string ) ) {
			 ...
			 * @return string
			 */
			$func_replace_callback = function ( $matches ) use ( $us_elm_context ) {

				// Check the current object type and ID

				// CASE: elements inside grid items
				if ( $us_elm_context == 'grid' ) {
					global $us_grid_object_type;

					// CASE: elements inside grid item showing a term
					if ( $us_grid_object_type == 'term' ) {
						global $us_grid_term;
						$object_id = $us_grid_term->term_id;
						$object_type = 'term';

						// Use the taxonomy slug as prefix in the term ID for ACF functions
						// https://www.advancedcustomfields.com/resources/get_field/
						$acf_object_id = $us_grid_term->taxonomy . '_' . $object_id;

						// CASE: elements inside grid item showing a singular post
					} else {
						$object_id = $acf_object_id = get_the_ID();
						$object_type = 'post';
					}

					// CASE: elements on an archive page itself (not inside grid items)
				} elseif (
					(
						is_tax()
						OR is_tag()
					...
					// Use the taxonomy slug as prefix in the term ID for ACF functions
					// https://www.advancedcustomfields.com/resources/get_field/
					$acf_object_id = $term->taxonomy . '_' . $object_id;

					// CASE: elements on a singular page itself (not inside grid items)
				} else {
					$object_id = $acf_object_id = get_the_ID();
					$object_type = 'post';
				}
				...
				if ( $matches[0] === '{{comment_count}}' AND $object_type === 'post' ) {
					...
					// Check if it is the ACF field, and get its value with respecting its "Return format"
					if ( function_exists( 'us_acf_get_field_object' ) ) {
						$acf_field = us_acf_get_field_object( $matches[1], $acf_object_id );
						$meta_value = us_arr_path( $acf_field, 'value', '' );

						// ... if not, use the default 'get_metadata'
					} else {
...
if ( ! function_exists( 'us_user_profile_html' ) ) {
	...
	function us_user_profile_html( $logout_redirect, $hidden = FALSE ) {
		...
		if ( empty( $logout_redirect ) ) {
			$logout_redirect = site_url( us_get_safe_var( 'REQUEST_URI' ) );
		}
	...
	function us_conditions_are_met( $conditions, $conditions_operator ) {
		...
		foreach( $conditions as $i => $condition ) {
			...
			if ( $condition_param === 'post_type' ) {
				...
				$condition_result = $func_compare_values( $current_post_id, us_arr_path( $condition, 'post_value' ), $mode, TRUE );

			} elseif ( $condition_param === 'page_url' ) {
				global $wp;
				$current_page_url = rawurlencode( home_url( $wp->request ) );
				...
				foreach( $terms as $term ) {
					...
					$condition_result = $func_compare_values( $term_value, us_strtolower( $condition_value ), $mode, TRUE );
				...
				if ( ! empty( $current_term ) ) {
					...
					// Use the taxonomy slug as prefix in the term ID for ACF functions
					// https://www.advancedcustomfields.com/resources/get_field/
					$acf_object_id = $current_term->taxonomy . '_' . $object_id;

				} else {
					$object_id = $acf_object_id = $current_post_id;
					$meta_type = 'post';
				}

				// Check if it is the ACF field, and get its value with respecting its "Returm format"
				if ( function_exists( 'us_acf_get_field_object' ) ) {
					$acf_field = us_acf_get_field_object( $meta_key, $acf_object_id );
					$meta_value = us_arr_path( $acf_field, 'value', '' );
				...
				$condition_result = $func_compare_values( $meta_value, us_arr_path( $condition, 'cf_value', '' ), us_arr_path( $condition, 'cf_mode', '=' ) );
				...
				$condition_result = $func_compare_values( $cart_total, $custom_value, us_arr_path( $condition, 'cart_total_mode' ) );
				...
				$condition_result = $func_compare_values( $wc_current_endpoint, us_arr_path( $condition, 'wc_account_endpoint', '' ), $mode );
				...
				$condition_result = $func_compare_values( $_current_time, $_custom_time, $mode );
			}
...
if ( ! function_exists( 'us_get_specific_classes_by_shortcode' ) ) {
	...
	function us_get_specific_classes_by_shortcode( $atts, $to_string = TRUE ) {
		...
		if ( isset( $atts['css'] ) AND $unique_classname = us_get_design_css_class( $atts['css'] ) ) {
			$css_classes[] = (string) $unique_classname;
		}
