...
return utftext}
!function($,undefined){var _window=window,_document=document,_undefined=undefined;if(_window.$usof===_undefined){_window.$usof={}}
if($usof.mixins===_undefined){$usof.mixins={}}
$usof.uniqid=function(prefix){return(prefix||'')+Math.random().toString(36).substr(2,9)};$usof.mixins.Events={on:function(eventType,handler){var self=this;if(self.$$events===_undefined){self.$$events={}}
if(self.$$events[eventType]===_undefined){self.$$events[eventType]=[]}
if($.isFunction(handler)){self.$$events[eventType].push(handler)}else{console.error('Invalid handler:',[eventType,handler])}
return self},off:function(eventType,handler){var self=this;if(self.$$events===_undefined||self.$$events[eventType]===_undefined){return self}
if(handler!==_undefined){var handlerPos=$.inArray(handler,self.$$events[eventType]);if(handlerPos!=-1){self.$$events[eventType].splice(handlerPos,1)}}else{self.$$events[eventType]=[]}
return self},has:function(eventType){var self=this;return self.$$events[eventType]!==_undefined&&self.$$events[eventType].length},trigger:function(eventType,extraParameters){var self=this;if(self.$$events===_undefined||self.$$events[eventType]===_undefined||self.$$events[eventType].length==0){return self}
...
return self}};$usof.getDynamicColors=function(){var self=this;if($.isPlainObject(self._dynamicColors)){return self._dynamicColors}
try{self._dynamicColors=JSON.parse(self.dynamicColors||'{}')}catch(e){self._dynamicColors={}}
return self._dynamicColors};$usof.field=function(row,options){var self=this;self.$document=$(_document);self.$row=$(row);self.$responsive=$('> .usof-form-row-responsive',self.$row);var data=self.$row.data()||{};self.type=self.$row.usMod('type');self.id=data.id;self.uniqid=$usof.uniqid();self.name=data.name;self.inited=data.inited||!1;self.$input=$('[name="'+data.name+'"]:not(.js_hidden)',self.$row);if(self.inited){return}
self.$$events={beforeShow:[],afterShow:[],change:[],beforeHide:[],afterHide:[]};if($usof.field[self.type]!==_undefined){for(var fn in $usof.field[self.type]){if(!$usof.field[self.type].hasOwnProperty(fn)||fn.substr(0,2)==='_$'){continue}
if(self[fn]!==_undefined){var parentFn='parent'+fn.charAt(0).toUpperCase()+fn.slice(1);self[parentFn]=self[fn]}
self[fn]=$usof.field[self.type][fn]}}
self.$document.on('usb.syncResponsiveState',self._usbSyncResponsiveState.bind(self));self.$row.data('usofField',self);var initEvent=function(){self.init(options);self.inited=!0;self.$row.data('inited',self.inited);self.off('beforeShow',initEvent);self._std=data.hasOwnProperty('std')?data.std:self.getCurrentValue();self.initResponsive()};self.on('beforeShow',initEvent)};$.extend($usof.field.prototype,$usof.mixins.Events,{init:function(){var self=this;if(self._events===_undefined){self._events={}}
self._events.change=function(){self.trigger('change',[self.getValue()])};self.$input.on('change',self._events.change);return self},isEditLive:function(){return!!this.$row.closest('.us-builder-panel-fieldset').length},initResponsive:function(){var self=this;if(!self.hasResponsive()){return}
...
self.$switchResponsive.on('click',self._$switchResponsive.bind(self));self.$responsive.on('click','[data-responsive-state]',self._$selectResponsiveState.bind(self))},hasResponsive:function(){return!!this.$responsive.length},isResponsive:function(){var self=this;return self.hasResponsive()&&self.$row.hasClass('responsive')},isResponsiveValue:function(value){var self=this;if(value){if(!$.isPlainObject(value)){value=self.toPlainObject(value)}
for(var i in self._states){if(value.hasOwnProperty(self._states[i])){return!0}}}
return!1},isValidState:function(state){return state&&(this._states||[]).indexOf(''+state)!==-1},isVCParamValue:function(){return this.$input.hasClass('wpb_vc_param_value')},toString:function(value){if(!$.isPlainObject(value)){return ''}
return usof_rawurlencode(JSON.stringify(value))},toPlainObject:function(value){try{value=JSON.parse(usof_rawurldecode(''+value)||'{}')}catch(e){value={}}
return value},getDefaultValue:function(){var self=this;return(self._std!==_undefined)?self._std:''},getValueByState:function(state,value){var self=this;if(self.isResponsiveValue(value)){if(!self.isValidState(state)){state='default'}
if(!$.isPlainObject(value)){value=self.toPlainObject(value)}
...
if(self.isResponsiveValue(value)){value=self.toPlainObject(value)}else{value={}}
value[state]=input;return self.toString(value)},getCurrentValue:function(){var self=this,value=self.getValue();return self.isResponsiveValue(value)?self.getValueByState(self._currentState,value):value},setCurrentValue:function(value,quiet){var self=this;if(self.isResponsive()){value=self.setValueByState(self._currentState,value,self.getValue())}
self.setValue(value,quiet)},getValue:function(){return this.$input.val()},setValue:function(value,quiet){var self=this;if(!self.isResponsive()&&self.isResponsiveValue(value)){self.$row.addClass('responsive')}
self.$input.val(value);if(!quiet){self.trigger('change',[value])}
if(self.isVCParamValue()){self.$input.trigger('change')}},_usbSyncResponsiveState:function(_,state){var self=this;if(!self.isResponsive()||!self.isValidState(state)){return}
...
return result},addGroup:function(index){this.$btnAddGroup.addClass('adding');var $groupPrototype=$(this.$groupPrototype.html());if(this.isForButtons&&index!==_undefined){this.$btnAddGroup.closest('.usof-form-group').find(' > .usof-form-group-item:eq('+parseInt(index)+')').after($groupPrototype)}else{this.$btnAddGroup.before($groupPrototype)}
...
if(!this.isBuilder){if($.isEmptyObject($usof.instance.valuesChanged)){clearTimeout($usof.instance.saveStateTimer);$usof.instance.$saveControl.usMod('status','notsaved')}
...
if(this.isEditLive){groupParams._setTitleForAccordion()}
...
return $target.get(0)}});$.fn.usofDragDrop=function(options){return this.each(function(){if(!$.data(this,'usofDragDrop')){$.data(this,'usofDragDrop',new $usof.dragDrop(this,options))}})}})(jQuery);!function($){var _window=window,_document=document,_undefined=undefined;$usof.ajaxUrl=$('.usof-container').data('ajaxurl')||ajaxurl;if($usof.mixins===_undefined){$usof.mixins={}}
$usof.mixins.Fieldset={initFields:function($container){var self=this;if(self.$fields===_undefined){self.$fields={}}
if(self.fields===_undefined){self.fields={}}
if(self.groups===_undefined){self.groups={}}
if(self.showIf===_undefined){self.showIf={}}
if(self.showIfDeps===_undefined){self.showIfDeps={}}
var groupElms=[];$('.usof-form-row, .usof-form-wrapper, .usof-form-group',$container).each(function(_,elm){var $field=$(elm),name=$field.data('name'),isRow=$field.hasClass('usof-form-row'),isGroup=$field.hasClass('usof-form-group'),isInGroup=$field.parents('.usof-form-group').length,$showIf=$field.find((isRow||isGroup)?'> .usof-form-row-showif':'> .usof-form-wrapper-content > .usof-form-wrapper-showif');if($field.closest('.usof-form-group-prototype').length){return}
if(isRow&&$field.closest('.usof-design-options').length&&!$container.is('[data-responsive-state-content]')){return}
self.$fields[name]=$field;if($showIf.length>0){self.showIf[name]=$showIf[0].onclick()||[];var showIfVars=self._getShowIfVariables(self.showIf[name]);for(var i=0;i<showIfVars.length;i ++){if(self.showIfDeps[showIfVars[i]]===_undefined){self.showIfDeps[showIfVars[i]]=[]}
self.showIfDeps[showIfVars[i]].push(name)}}
if(isRow&&(!isInGroup||self.isGroupParams)){self.fields[name]=$field.usofField(elm)}else if(isGroup){self.groups[name]=$field.usofGroup(elm)}});for(var fieldName in self.showIfDeps){if(!self.showIfDeps.hasOwnProperty(fieldName)||self.fields[fieldName]===_undefined){continue}
...
if(self._defaultValues===_undefined){self._defaultValues=self.getValues()}},updateVisibility:function(fieldName,isAnimated,isCurrentShown){var self=this;if(!fieldName||!self.showIfDeps[fieldName])return;if(isAnimated===_undefined){isAnimated=!0}
if(isCurrentShown===_undefined){isCurrentShown=!0}
var isPrevShown=self.$fields[fieldName].data('isShown');self.showIfDeps[fieldName].map(function(depFieldName){var field=self.fields[depFieldName],$field=self.$fields[depFieldName],isShown=self.getCurrentShown(depFieldName),shouldBeShown=self.executeShowIf(self.showIf[depFieldName],self.getValue.bind(self));if((!shouldBeShown&&isShown)||!isCurrentShown){isShown=!1}else if(shouldBeShown&&!isShown){isShown=!0}
if(isPrevShown!==_undefined){isShown=isPrevShown&&isShown}
$field.stop(!0,!1).data('isShown',isShown);if(isShown){self.fireFieldEvent($field,'beforeShow');$field.show();self.fireFieldEvent($field,'afterShow');if(field instanceof $usof.field){field.trigger('change',[field.getValue()])}}else{self.fireFieldEvent($field,'beforeHide');$field.hide();self.fireFieldEvent($field,'afterHide')}
if(!!self.showIfDeps[depFieldName]){self.updateVisibility(depFieldName,isAnimated,isShown)}})},getCurrentShown:function(fieldName){var self=this;if(!fieldName||!self.$fields[fieldName])return!0;var $field=self.$fields[fieldName],isShown=$field.data('isShow');if(isShown===_undefined){isShown=$field.css('display')!=='none'}
return!!isShown},_getShowIfVariables:function(condition){var self=this;if(!$.isArray(condition)||condition.length<3){return[]}else if($.inArray(condition[1].toLowerCase(),['and','or'])!=-1){var vars=self._getShowIfVariables(condition[0]),index=2;while(condition[index]!==_undefined){vars=vars.concat(self._getShowIfVariables(condition[index]));index=index+2}
return vars}else{return[condition[0]]}},executeShowIf:function(condition,getValue){var self=this,result=!0;if(!$.isArray(condition)||condition.length<3){return result}else if($.inArray(condition[1].toLowerCase(),['and','or'])!=-1){result=self.executeShowIf(condition[0],getValue);var index=2;while(condition[index]!==_undefined){condition[index-1]=condition[index-1].toLowerCase();if(condition[index-1]=='and'){result=(result&&self.executeShowIf(condition[index],getValue))}else if(condition[index-1]=='or'){result=(result||self.executeShowIf(condition[index],getValue))}
index=index+2}}else{var value=getValue(condition[0]);if(value===_undefined){return!0}
...
return result},fireFieldEvent:function($container,trigger){if(!$container.hasClass('usof-form-row')){$container.find('.usof-form-row').each(function(_,block){var $block=$(block),isShown=$block.data('isShown');if(isShown===_undefined){isShown=($block.css('display')!='none')}
if(!isShown&&['beforeShow','afterShow','beforeHide','afterHide'].indexOf(trigger)!==-1){return}
if($block.data('usofField')==_undefined){return}
$block.data('usofField').trigger(trigger)})}else if($container.data('usofField')instanceof $usof.field){$container.data('usofField').trigger(trigger)}},getValue:function(id){var self=this;if(self.fields[id]===_undefined){return _undefined}
return self.fields[id].getValue()},setValue:function(id,value,quiet){var self=this;if(self.fields[id]===_undefined){return}
...
if(quiet){for(var fieldName in self.showIfDeps){if(!self.showIfDeps.hasOwnProperty(fieldName)||self.fields[fieldName]===_undefined){continue}
self.updateVisibility(fieldName,!1);}}},prepareIconTag:function(iconValue){iconValue=iconValue.trim().split('|');if(iconValue.length!=2){return ''}
...
return iconTag}};$usof.GroupParams=function(container){this.$container=$(container);this.$group=this.$container.closest('.usof-form-group');this.group=this.$group.data('name');this.isGroupParams=!0;this.isBuilder=!!this.$container.parents('.us-bld-window').length;this.isForButtons=this.$group.hasClass('preview_button');this.isForFormElms=this.$group.hasClass('preview_input_fields');this.initFields(this.$container);this.fireFieldEvent(this.$container,'beforeShow');this.fireFieldEvent(this.$container,'afterShow');this.accordionTitle=(this.$group.data('accordion-title')!=_undefined)?decodeURIComponent(this.$group.data('accordion-title')):'';if(!this._isEmptyAccordionTitle()){for(var fieldId in this.fields){if(!this.fields.hasOwnProperty(fieldId)){continue}
...
if(this.group!==_undefined&&$usof.instance.groups[this.group]instanceof $usof.Group){$usof.instance.valuesChanged[this.group]=$usof.instance.groups[this.group].getValue()}}.bind(this))}}
this.$container.data('usofGroupParams',this);if(this.isForButtons){this.$buttonPreview=this.$container.find('.usof-form-group-item-title .usof-btn-preview');new $usof.ButtonPreview(this.$buttonPreview)}else if(this.isForFormElms){new $usof.FormElmsPreview(this.$container.find('.usof-input-preview'))}};$.extend($usof.GroupParams.prototype,$usof.mixins.Fieldset,{_isEmptyAccordionTitle:function(){return this.accordionTitle===_undefined||this.accordionTitle===''},_setTitleForAccordion:function(){if(this._isEmptyAccordionTitle())return;this.$title=this.$container.find('.usof-form-group-item-title');if(this.isForButtons){this.$title=this.$title.find('.usof-btn-label')}
...
$(_document.body).trigger('usof_mm_save')}.bind(this));}};$.extend(USOF_Meta.prototype,$usof.mixins.Fieldset,{});var USOF=function(container){if(_window.$usof===_undefined){_window.$usof={}}
...
if(this.$sections[this.active]!==_undefined){this.fireFieldEvent(this.$sections[this.active],'beforeShow');this.fireFieldEvent(this.$sections[this.active],'afterShow')}
...
this.headerAreaSize=this.$header.offset().top+this.$header.outerHeight();this.scroll()},openSection:function(sectionId){if(sectionId==this.active||this.$sections[sectionId]===_undefined){return}
if(this.$sections[this.active]!==_undefined){this.hideSection()}
...
if(light<178){this.$input.addClass('white')}else{this.$input.removeClass('white')}}}}(jQuery);!function($,undefined){var _window=window,_undefined=undefined;if(_window.$usof===_undefined){return}
$usof.field.css=$usof.field.html={init:function(){var self=this;self._params={};self.editor=null;self.editorDoc=null;self._events={editorChange:function(){self.parentSetValue(self.getValue())},editorFocused:function(_,e){self.$row.toggleClass('focused',e.type==='focus')}};if(wp.hasOwnProperty('codeEditor')){var $params=$('.usof-form-row-control-params',self.$row);if($params.is('[onclick]')){self._params=$params[0].onclick()||{};$params.removeAttr('onclick')}
if(self._params.editor!==!1){self.editor=wp.codeEditor.initialize(self.$input[0],self._params.editor||{});self.editorDoc=self.editor.codemirror.getDoc();self.setValue(self.$input.val());self.editor.codemirror.on('focus',self._events.editorFocused);self.editor.codemirror.on('blur',self._events.editorFocused)}}else{self.$input.on('keyup',function(){self.parentSetValue(self.getValue());self.setValue(self.$input.val())})}},setValue:function(value){var self=this;if(!!self._params&&self._params.hasOwnProperty('encoded')&&self._params.encoded){value=usof_rawurldecode(usof_base64_decode(value))}
if(self.editor!==_undefined&&wp.hasOwnProperty('codeEditor')){self.editorDoc.off('change',self._events.editorChange);if(!!self.pid){clearTimeout(self.pid)}
self.pid=setTimeout(function(){self.editorDoc.cm.refresh();self.editorDoc.setValue(value);self.editorDoc.on('change',self._events.editorChange);clearTimeout(self.pid)},1)}},getValue:function(){var self=this,value=self.editor!==_undefined&&wp.hasOwnProperty('codeEditor')?self.editorDoc.getValue():self.$input.val();if(self._params!==_undefined&&self._params.encoded){value=usof_base64_encode(usof_rawurlencode(value))}
...
return '<span class="usof-autocomplete-selected" data-key="'+key+'">\
			...
			</span>'},getValue:function(){return(this.$value instanceof $)?this.$value.val():''},setValue:function(value,quiet){this.$value.val(value);this._initValues.call(this);if(!quiet){this.trigger('change',[value])}}}}(jQuery);!function($,undefined){var _window=window,_undefined=undefined;if(_window.$usof===_undefined){return}
$usof.field.radio=$usof.field.imgradio={init:function(){var self=this;self.$radio=$('input[type=radio]',self.$row);self._events={changeCurrentValue:self._changeCurrentValue.bind(self),syncRadioButtons:self._syncRadioButtons.bind(self)};self.$row.on('click','input[type=radio]',self._events.changeCurrentValue);if(self.hasResponsive()){self.on('setResponsiveState',self._events.syncRadioButtons)}},_changeCurrentValue:function(e){var self=this,value=(e.target||{}).value;if(value===_undefined){value=self.getDefaultValue()}
self.setCurrentValue(value)},_syncRadioButtons:function(){var self=this;self.$radio.removeAttr('checked').filter('[value="'+self.getCurrentValue()+'"]').prop('checked',!0)},setValue:function(value,quiet){var self=this;self.parentSetValue(''+value);self._syncRadioButtons()}}}(jQuery);!function($,undefined){var _window=window,_document=document,_undefined=undefined;if(_window.$usof===_undefined){return}
$usof.field.link={init:function(options){var self=this;self.parentInit(options);self.$url=$('input[type="text"]:first',self.$row);self.$target=$('input[type="checkbox"]:first',self.$row);self.format=(''+self.$input.data('format'));if(['json','jsons','serialized'].indexOf(self.format)===-1){self.format='jsons'}
if(!self.$target.is('[name]')){self.$target.attr('name',$usof.uniqid())}
self._events={applyChange:self._applyChange.bind(self),exampleClick:self._exampleClick.bind(self),};self.$row.on('click','.usof-example',self._events.exampleClick);self.$url.on('change',self._events.applyChange);self.$target.on('change',self._events.applyChange)},_applyChange:function(){var self=this,value=self.getValue();self.$input.val((typeof value!=='string')?JSON.stringify(value):value);self.trigger('change',[value])},_exampleClick:function(e){var $example=$(e.target).closest('.usof-example');if(!$example.length)return;this.$url.val($example.text())},getValue:function(){var self=this;if(!self.inited)return;var value={url:usof_rawurlencode(self.$url.val()),target:self.$target.is(':checked')?'_blank':''};if(self.format==='serialized'){var result='';for(var k in value){if(value.hasOwnProperty(k)&&value[k]){result+=k+':'+value[k]+'|'}}
if(result.length>0){result=result.substring(0,result.length-1)}
return result}else if(self.format==='jsons'&&$.isPlainObject(value)){return JSON.stringify(value)}
return value},setValue:function(value,quiet){var self=this;if(!self.inited)return;var newValue={url:'',target:''};if(self.format==='serialized'&&((''+value).substr(0,4)==='url:'||(''+value).substr(0,7)==='target:'||(''+value).indexOf('|')!==-1)){var pairs=value.trim().split('|');for(var i=0;i<pairs.length;i ++){var param=pairs[i].split(':');if(param[0]&&param[1]){newValue[param[0]]=usof_rawurldecode(param[1])}}}else if(value&&self.format==='jsons'){newValue=JSON.parse(value)}else if($.isPlainObject(value)){newValue=$.extend(newValue,value||{})}
if(!!newValue.url){newValue.url=usof_rawurldecode(newValue.url)}
self.$url.val(newValue.url);self.$target.prop('checked',(newValue.target==='_blank'));if(typeof value!=='string'){value=JSON.stringify(value)}
self.$input.val(value)}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
value=this.renderValue(value);this.parentSetValue(value,quiet)}}}(jQuery);!function($,undefined){var _window=window,_undefined=undefined;if(_window.$usof===_undefined){return}
...
return value?1:0},getValue:function(){var self=this;return self._toNumber(self.$input.is(':checked'))},setValue:function(value,quiet){var self=this;value=self._toNumber(value);self.$input.val(value).prop('checked',value);if(!quiet){self.trigger('change',[value])}}}}(jQuery);!function($,undefined){var _window=window,_undefined=undefined;if(_window.$usof===_undefined){return}
$usof.field.text=$usof.field.textarea={init:function(){var self=this;self.$row.on('click','.usof-example',self._setExampleValue.bind(self));self.$input.on('change keyup',function(){self.trigger('change',[self.getValue()])})},_setExampleValue:function(e){this.setValue($(e.target).closest('.usof-example').html()||'')}}}(jQuery);!function($,undefined){var _window=window,_undefined=undefined;if(_window.$usof===_undefined){return}
$usof.field.editor={init:function(){var self=this;self.$container=$('.usof-editor',self.$row);self.$container.find('script.usof-editor-template').remove();self.hasUndoRedo=!1;self.originalEditorId=self.$input.data('editor-id')||'usof_editor';self.originalEditorSettings=_window.tinyMCEPreInit.mceInit[self.originalEditorId]||{};var $editorSettings=$('.usof-editor-settings',self.$row);self.editorSettings=$editorSettings.is('[onclick]')?$editorSettings[0].onclick()||{}:{};self.editorId=self.originalEditorId+$usof.uniqid();self.$input.attr('id',self.editorId);self._events={changeField:self._changeField.bind(self),changeTinymceContent:self._changeTinymceContent.bind(self)};self.$container.on((self.isEditLive()?'input':'change'),'textarea',self._events.changeField);self.initEditor()},initEditor:function(){if(!_window.wp||!_window.wp.editor)return;var self=this;_window.tinymce.on('AddEditor',function(e){var _editor=e.editor;if(_editor.id!==self.editorId)return;var eventName='NodeChange '+(self.isEditLive()?'input':'change');_editor.off(eventName).on(eventName,self._events.changeTinymceContent);if(self.isEditLive()){_editor.on('keydown',self.trigger.bind(self,'tinyMCE.Keydown')).on('BeforeAddUndo',function(){return!1})}},!0);var pid=setTimeout(function(){var editorSettings={quicktags:!0,tinymce:self.editorSettings.tinymce||{},mediaButtons:(self.editorSettings.media_buttons!==_undefined)?self.editorSettings.media_buttons:!0},qtSettings={id:self.editorId,buttons:"strong,em,link,block,del,ins,img,ul,ol,li,code,more,close"},settingsFields=['content_css','toolbar1','toolbar2','toolbar3','toolbar4','theme','skin','language','formats','relative_urls','remove_script_host','convert_urls','browser_spellcheck','fix_list_elements','entities','entity_encoding','keep_styles','resize','menubar','branding','preview_styles','end_container_on_empty_block','wpeditimage_html5_captions','wp_lang_attr','wp_keep_scroll_position','wp_shortcut_labels','plugins','wpautop','indent','tabfocus_elements','textcolor_map','textcolor_rows',];settingsFields.forEach(function(setting,index){if(self.originalEditorSettings[setting]!==_undefined){editorSettings.tinymce[setting]=self.originalEditorSettings[setting]}});editorSettings.tinymce.setup=$.noop;_window.wp.editor.initialize(self.editorId,editorSettings);_window.quicktags(qtSettings);self.switchEditors('tinymce');clearTimeout(pid)},1)},switchEditors:function(mode){var self=this;mode=(''+mode).toLowerCase();$('#'+self.editorId+'-'+(mode==='tinymce'?'tmce':'html'),self.$container).trigger('click')},_changeField:function(e){this.trigger('change',e.currentTarget.value)},_changeTinymceContent:function(e){var self=this,mceValue=''+_window.tinymce.get(self.editorId).getContent(),currentValue=''+self.getValue();if(currentValue===mceValue){return}
...
$usof.field.upload={init:function(options){this.attachmentAtts={};this.parentInit(options);this.$upload=$('.usof-upload',this.$row);this.previewType=this.$upload.usMod('preview');this.isMultiple=this.$upload.hasClass('is_multiple');this.thumbSize=(this.isMultiple)?'thumbnail':'medium';this.$btnAdd=$('.ui-icon_add',this.$row);this.$previewContainer=$('.usof-upload-preview',this.$row);this.placeholder=$('input[name="placeholder"]',this.$row).val();this.$btnAdd.on('click',this.openMediaUploader.bind(this));var $i18n=$('.usof-upload-i18n',this.$row);this.i18n={};if($i18n.length>0){this.i18n=$i18n[0].onclick()||{};$i18n.remove()}
this._initDynamicBtns();if(this.isMultiple&&this.previewType==='image'){this.$dragshadow=$('<div class="usof-dragshadow"></div>');this.$body=$(_document.body);this.$window=$(_window);this._events={_maybeDragMove:this._maybeDragMove.bind(this),_dragMove:this._dragMove.bind(this),_dragEnd:this._dragEnd.bind(this)};this.$row.on('mousedown','.usof-upload-preview-file',this._dragStart.bind(this)).on('dragstart',function(e){e.preventDefault()})}},_initDynamicBtns:function(){this.$btnsDeleteImage=$('.ui-icon_delete',this.$row);this.$btnsDeleteImage.on('click',function(e){var $target=$(e.target),$img=$target.closest('.usof-upload-preview-file'),imgID=($img.attr('data-id'))?$img.attr('data-id'):null;if(imgID){var value=this.getValue().trim().split(',');if($.inArray(imgID,value)!==-1){value.splice($.inArray(imgID,value),1);$img.remove();if(value.length){this.parentSetValue(value)}else{this.setValue('')}}}}.bind(this));if(!this.isMultiple){$previewItem=$('.usof-upload-preview-file',this.$previewContainer);if($previewItem.length){$previewItem.on('click',this.openMediaUploader.bind(this))}}},_dragStart:function(e){if(this._hasClass(e.target,'ui-icon_delete')){return}
e.stopPropagation();this.$draggedElm=$(e.target).closest('.usof-upload-preview-file');this.detached=!1;this._updateBlindSpot(e);this.elmPointerOffset=[parseInt(e.pageX),parseInt(e.pageY)];this.$body.on('mousemove',this._events._maybeDragMove);this.$window.on('mouseup',this._events._dragEnd)},_updateBlindSpot:function(e){this.blindSpot=[e.pageX,e.pageY]},_isInBlindSpot:function(e){return Math.abs(e.pageX-this.blindSpot[0])<=20&&Math.abs(e.pageY-this.blindSpot[1])<=20},_maybeDragMove:function(e){e.stopPropagation();if(this._isInBlindSpot(e)){return}
this.$body.off('mousemove',this._events._maybeDragMove);this._detach();this.$body.on('mousemove',this._events._dragMove)},_detach:function(e){var offset=this.$draggedElm.offset();this.elmPointerOffset[0]-=offset.left;this.elmPointerOffset[1]-=offset.top;this.$dragshadow.css({width:this.$draggedElm.outerWidth(),height:this.$draggedElm.outerHeight()}).insertBefore(this.$draggedElm);this.$draggedElm.css({position:'absolute','pointer-events':'none',zIndex:10000,width:this.$draggedElm.width(),height:this.$draggedElm.height()}).css(offset).appendTo(this.$body);this.detached=!0},_dragMove:function(e){e.stopPropagation();this.$draggedElm.css({left:e.pageX-this.elmPointerOffset[0],top:e.pageY-this.elmPointerOffset[1]});if(this._isInBlindSpot(e)){return}
var elm=e.target;for(var level=0;level<=2;level ++,elm=elm.parentNode){if(this._isShadow(elm)){return}
...
this.frame=wp.media(mediaSettings);this.frame.on('open',function(){var value;this.frame.state().get('selection').reset();if(this.isMultiple){value=this.getValue().trim().split(',');$.each(value,function(index,file){var fileID=parseInt(file);if(fileID){this.frame.state().get('selection').add(wp.media.attachment(fileID))}}.bind(this))}else{value=parseInt(this.getValue());if(value){this.frame.state().get('selection').add(wp.media.attachment(value))}}}.bind(this));this.frame.on('select',function(){if(this.isMultiple){var attachments=[];this.frame.state().get('selection').each(function(attachment){if(!!attachment.attributes.url){attachments.push(attachment.id)}});this.setValue(attachments)}else{var attachment=this.frame.state().get('selection').first();if(!!attachment.attributes.url){this.attachmentAtts=attachment.attributes;this.setValue(attachment.id,!1)}}}.bind(this))}
...
this.$importBtnCopy.prop('disabled',!this.getValue());clearTimeout(pid)}.bind(this),1);this.$container.find('.usof-responsive-buttons').toggleClass('hidden',!this.extStates.length);this.$container.on('click','[data-accordion-id]',this._events.toggleAccordion.bind(this)).on('click','.usof-design-options-reset',this._events.resetValues.bind(this)).on('click','.usof-design-options-responsive',this._events.toggleResponsive.bind(this)).on('click','[data-responsive-state]',this._events.selectResponsiveState.bind(this));this.$import.on('click','button[data-action]',this._events.importActions.bind(this))},_events:{changeValue:function(field){var resultValue={},valueStateChanged={},enabledResponsives={};$.each(this.groupParams,function(responsiveState,groupParams){if('default'===responsiveState){for(var k in groupParams.fields){enabledResponsives[k]=!!$(groupParams.fields[k].$row).closest('[data-accordion-content]').prev('[data-accordion-id].responsive').length}}
...
$.each(this.groupParams,function(responsiveState,groupParams){groupParams.setValues(this.defaultValues[responsiveState]||{},!0)}.bind(this));this.setValue(value);this.$import.removeClass('show_input');this.$importBtnCopy.prop('disabled',!value)}},selectResponsiveState:function(e){var responsiveState=$(e.currentTarget).data('responsive-state');this.switchResponsiveState(responsiveState);this.trigger('syncResponsiveState',responsiveState);this._lastSelectedResponsiveState=responsiveState}},_usbSyncResponsiveState:function(_,state){this._lastSelectedResponsiveState=state||'default';this.switchResponsiveState(this._lastSelectedResponsiveState)},switchResponsiveState:function(responsiveState,hidden){if(!responsiveState)return;var hasResponsiveClass=!hidden?'.responsive':':not(.responsive)'
var $target=$('[data-accordion-id]'+hasResponsiveClass,this.$container).next('[data-accordion-content]').find('[data-responsive-state="'+responsiveState+'"]');$target.siblings().removeClass('active');$target.addClass('active').closest('.usof-design-options-content').find('> [data-responsive-state-content]').addClass('hidden').filter('[data-responsive-state-content="'+responsiveState+'"]').removeClass('hidden')},_isValidValue:function(value){try{value=this.toPlainObject((''+value).trim());for(var i in this.states){var state=this.states[i];if(state&&!!value[state]){return!0}}}catch(err){}
...
change=(change||JSON.stringify(values)!==JSON.stringify(currentGroupValues[groupName][responsiveState]))}.bind(this));this.$container.find('[data-accordion-id='+groupName+']').toggleClass('changed',change)}.bind(this))},getValue:function(){var value=$.trim(this.$input.val());if(!this.hasStringValue&&value&&typeof value==='string'){value=this.toPlainObject(value)}
return value},setValue:function(value,quiet){var savedValues={};if(typeof value==='string'){savedValues=this.toPlainObject(value)}else if($.isPlainObject(value)){savedValues=value}
