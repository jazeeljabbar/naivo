...
foreach ( $filter_items as $filter_item ) {
	...
	if ( $enable_toggles ) {
		$_item_title_helper = ' <span class="w-filter-item-reset" aria-hidden="true">' . strip_tags( __( 'Reset', 'us' ) ) . '</span>';
...
if ( ! empty( $mobile_width ) AND $output_items !== '' ) {
	...
	$output .= '<button class="w-btn us-btn-style_' . $btn_style . '">';
	...
	if ( ! empty( $mobile_button_style ) ) {
		$mobile_button_atts['class'] .= ' w-btn us-btn-style_' . us_maybe_get_button_style( $mobile_button_style );
	}
