...
if ( $us_bg_image_source == 'media' ) {
	...
	// Get ID from ACF fields
	if (
		function_exists( 'us_acf_get_field_object' )
		AND $acf_field = us_acf_get_field_object( $us_bg_image_source )
	) {
	...
	if ( empty( $_attachment_id ) ) {
		$meta_type = is_archive()
			? 'term'
			: 'post';
		$_attachment_id = get_metadata( $meta_type, $object_id, $us_bg_image_source, TRUE );
	}

	// If it is an ID - getting attachment by ID
	if ( is_numeric( $_attachment_id ) ) {
		$image_src = wp_get_attachment_image_src( $_attachment_id, 'full' );
		// Otherwise checking it is a scalar value (not array or object) and using value as URL
	} elseif ( is_scalar( $_attachment_id ) ) {
		// for acf image url
		$image_src = array( (string) $_attachment_id, /* width */FALSE, /* height */FALSE );
	}
}
