...
} elseif ( preg_match( '/^acf_(gallery|posts)_(option[\/|\|])?/', $post_type, $matches ) ) {
	if ( ! is_singular() ) {
		$current_post_id = get_queried_object_id();
	}

	$post__in = array();
	$meta_key = str_replace( $matches[ /* full prefix */0 ], us_arr_path( $matches, /* option| */'2' , '' ), $post_type );

	// Get data from the option page.
	if ( ! empty( $matches[ /* option| */2 ] ) ) {
		if (
			function_exists( 'us_acf_get_field_object' )
			AND $acf_field = us_acf_get_field_object( $meta_key, $current_post_id )
		) {
			$_return_format = us_arr_path( $acf_field, 'return_format' );
			if ( $_return_format === 'array' ) {
				$post__in = wp_list_pluck( $acf_field[ 'value' ], 'id' );
			}
			else if ( $_return_format === 'id' ) {
				$post__in = $acf_field[ 'value' ];
			}
			else if ( $_return_format === 'object' ) {
				foreach( (array) $acf_field[ 'value' ] as $post_object ) {
					$post__in[] = $post_object->ID;
				}
			}
			else {
				// $_return_format === 'url'
		...
		$post__in = get_post_meta( $current_post_id, $meta_key, /* single */TRUE );

	} else {
		$post__in = get_term_meta( $current_post_id, $meta_key, /* single */TRUE );
	}
