...
class VI_WOO_CART_ALL_IN_ONE_Frontend_Menu_Cart {
	...
	public function create_primary_menu_cart_item( $menu, $args ) {
		...
		if ( !empty( $mc_menu_display ) || !isset($args['theme_location']) || $args['theme_location'] !=='primary') {
			return $menu;
		}
		if (!strstr( $menu, 'vi-wcaio-menu-cart' )) {
			$menu = preg_replace( "/(<\/li\>(?!.*<\/li>))/s", $this->get_menu_cart_html(), $menu );
		}
		return $menu;
	}
	public function create_menu_cart( $items, $args ) {
		...
		if ( ! strstr( $items, 'vi-wcaio-menu-cart' ) ) {
	...
	public static function get_menu_cart_text( $display_style, $cart_total, $cart_content_count ) {
		...
		echo sprintf( '<span class="vi-wcaio-menu-cart-text vi-wcaio-menu-cart-text-%s%s">%s</span>', esc_attr( $display_style ),esc_attr(WC()->cart->is_empty()?' vi-wcaio-menu-cart-empty':''), wp_kses_post( $text ) );
	}
